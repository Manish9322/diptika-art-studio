...
import { Briefcase, BarChart3, ChevronRight, Settings, Image as ImageIcon } from 'lucide-react';
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onNavigate, onLogout }) => {
  ...
  const stats = [
    ...
    { label: 'Total Views', value: '12.4k', icon: <BarChart3 size={20} /> },
  ];
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8 mb-12 sm:mb-16">
        {stats.map((stat, i) => (
          <div key={i} className="bg-white p-6 sm:p-8 border border-zinc-100 shadow-sm flex items-center justify-between">
            <div>
      ...
      <div className="mt-16 pt-12 border-t border-zinc-100 flex flex-col sm:flex-row items-center justify-between gap-6">
        ...
        <div className="flex items-center space-x-6">
           <Settings size={18} className="text-zinc-200 hover:text-zinc-900 cursor-pointer transition-colors" />
        </div>
      </div>
    </div>
  );
